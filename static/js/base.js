...
$(function(){
	...
	$(window).scroll(function(){
		...
		if(screenWidth>1300){
			...
			if(!scrollBool){
			$(".header").stop(false,false).animate({"height":"66px"},200);
			$(".header .nav").stop(false,false).animate({"height":"66px"},200);
			$(".header .nav .child").stop(false,false).animate({"height":"66px"},200).css({"line-height":"66px"});
			...
			$(".header .nav .child .childContent").css("top","66px");
			$(".header .headerLogo").stop(false,false).animate({"top":"5px",height:"54px"},200);
			$(".header .gs").stop(false,false).animate({"margin-top":"30px","margin-left":"200px"},200);
			$(".crumbs").animate({"padding-top":"66px"},200);
			scrollBool=true;
			}
		}else{
			if(scrollBool){
			...
			$(".header .headerLogo").stop(false,false).animate({"top":"8px",height:"60px"},200);
			$(".header .gs").stop(false,false).animate({"margin-top":"50px","margin-left":"200px"},200);
...
$("#gs").click(function(e){//切换站点
	   $(this).find(".fgs").toggle();
...
$(".fgs").on("click", function(e){
    e.stopPropagation();
});


/*侧边栏*/
$(window).scroll(function(){//侧边栏滚动
	if($(this).scrollTop()>=200){
		   $(".side-bar").css({"opacity":"1"}).show();
		}else{
			$(".side-bar").css({"opacity":"0"}).hide();
			}
	}
);
